...
public class UserUpdateService {
	 ...
	 * 更新文件
	 */
	// public static File updateFile = new
	// File("/sdcard/tvhelperapk/tvhelper.apk");
	public static File updateFile = new File("/data/data/com.changhong.ghliveandreplay/GHLiveAndReplay.apk");
	 ...
	 * 
	 * @param context
	 *            context information which need activity pass in
	 * @param handler
	 *            handler which need declare in activity and pass in please
	 *            check message code which definition above
	 *            {@link com.changhong.setting.activity.SettingActivity}
	 * @param m_pDialog
	 *            progress dialog which need declare in activity and pass in
	 */
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					PackageInfo instalPMInfo = pm.getPackageArchiveInfo(updateFile.getAbsolutePath().toString(),
					...
					// 在比较本地程序和安装APK的版本
					PackageInfo localPMInfo = pm.getPackageInfo(context.getPackageName(),
	...
	public BroadcastReceiver updateReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context mContext, Intent mIntent) {
			...
			if (mIntent.getAction().equals("SETTING_UPDATE_DOWNLOAD")) {
				...
				Dialog dialog = DialogUtil.showAlertDialog(context, "电视助手版本升级", content,
						...
						new Thread(new Runnable() {
							...
							public void run() {
								...
								try {
									...
									// 不停的更新下载的状态
									UpdateLogService preferenceService = new UpdateLogService(context);
									while (!UserUpdateService.THREAD_ONE_FINISHED
											|| !UserUpdateService.THREAD_TWO_FINISHED) {
										Thread.sleep(100);
				...
				if (directlyInstall) {
					...
					Dialog dialog = DialogUtil.showAlertDialog(context, "已经为您准备好更新", "最新的版本已经下载完成,是否安装更新？",
							new DialogBtnOnClickListener() {
